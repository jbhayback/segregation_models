...
# Gets the raw input data from Input_data.csv
...
if (validate_data_for_invalid_chars()):
    cleaned_input_data = convert_char_seq_to_numeric_grid()
    population_size = int(cleaned_input_data.shape[0] * cleaned_input_data.shape[1])
    ...
    if st.sidebar.button('Calculate Index of Dissimilarity'):
        is_valid_row_col_input = validate_row_column_inputs(input_row, input_col)
        if is_valid_row_col_input[0]:
            ...
            for data_per_tract in data_tracts:
                st.text('Data Grid for Tract ' + str(tract_number))
                st.dataframe(data_per_tract)
                tract_number += 1
                partial_indices.append(dissimilarity.calculate_partial_index(data_per_tract))
            ...
            if is_valid_row_col_input[1] == "NOT_MULTIPLE":
                ...
                st.error("Cannot split the data grid with equal number of chars per tract/splice based on the input row or column.")
